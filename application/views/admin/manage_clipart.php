                                    ...
                                    <table id="order-table" class="table table-striped table-bordered nowrap dataTable" role="grid" aria-describedby="order-table_info">
											...
											<tbody>
												...
												<tr>
													<td><?= $counter ?></td>
													<td><?= $clipart->name ?></td>
											<?php	$q = $this->db->query("Select * from categories_reference where item_id= '$clipart->id;'");
											    $cat = $q->row();
											    $cat_name = $this->db->query("Select * from categories where id= '$cat->category_id;'");
											    $category = $cat_name->row()
											    ;?>
													<td><?= $category->name ?></td>
													<!--<td><?= $clipart->tags ?></td>-->
													...
													<td>
														...
														<button
														 ...
														 <form action="" method="post" class="del_button">
													          <input type="hidden" name="id" value="<?=$clipart->id?>">
                                                        <button
														  class="btn waves-effect waves-light btn-danger btn-xs btn-icon btn-custom" type="submit" onclick="return confirm('Are you sure to delete ?')" name="delete" value="cliparts"><i
