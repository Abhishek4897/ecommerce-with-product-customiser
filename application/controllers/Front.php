...
class Front extends CI_Controller {
    ...
    function designerList(){
           ...
           $out =  $out.'<div class="form-group mb-0">
                        ...
                        <div class="form-check">
                            <label class="custom-checkbox-designer" for="desktop'.$d->id.'">'.$d->user_fullname.'
                             <input class="designer-search-checkbox designers" onclick ="category_data_load()" type="checkbox" id="desktop'.$d->id.'" value="'.$d->id.'"/>
                           ...
                           $out1 =  $out1.'<div class="form-group mb-0">
                            ...
                            <label class="custom-checkbox-designer" for="mdesigner'.$d->id.'">'.$d->user_fullname.'
                              <input class="designer-search-checkbox designers" onclick ="category_data_load()" type="checkbox" id="mdesigner'.$d->id.'" value="'.$d->id.'"/>
                            <span class="checkmark"><i class="fa fa-check"></i></span>
                            </label>
                            </div>
                        </div>';
        $i++;
            ...
            $out = '<center>No designer found</center>';
            $out1 = '<center>No designer found</center>';
        }
            ...
            if(count($data)>0) {
                ...
                foreach ($data  as $items):
                    ...
                    if($items->size!="" && $cart->id==2){
                        $size = '<div class="variation-name">SIZE</div>&emsp;&emsp;&emsp;&emsp;<div class="variation-value">'. $items->size.'</div>';
                        $msize ='<div class="cart-variation">Size:&emsp;&emsp;&emsp;&emsp;'. $items->size.'</div>'  ;
                    } else {
                        $size='';
                        $msize='';
                    }
                    ...
                    <h5>'.ucwords(strtolower($cart->product_name)).' </h5>
                    <span class="rupess"> Rs. '.$p->total.'/-</span>
                    <div style="">
                    ...
                    <div class="variation-value">&emsp;'.$items->qty.'</div>
                    </div>
                    
                    </div>
                    <a href="'.base_url().'customizer?product='.$cart->id.'&cart_id='.$items->id.'" style="color:green"><span class="cart-control" style="padding-right: 12px;color:green"><i class="fas fa-edit"></i> EDIT</span></a>
                    </div>
                    ...
                    <a href="'.base_url().'customizer?product='.$cart->id.'&cart_id='.$items->id.'" class="cart-control  float-right"><span class="cart-control text-success float-right"><i class="fas fa-edit"></i> EDIT</span></a>
                    </div>
                    </div>
            ...
            if(count($cart)>0){
                ...
                foreach ($cart as $items): 
                    ...
                    $out  = $out.'
                        ...
                        <div class="product-name">
                        <a href="'.base_url().''.$slug.'/'.$t_id[0].'">  
                        <h6 class="text-black">'.ucwords(strtolower($cart->product_name)).' </h6>
                        </a>
                        ...
                        <a href="'.base_url().'customizer?product='.$cart->id.'&cart_id='.$i.'"><span class="cart-control text-success" style="padding-right: 12px;"><i class="fas fa-edit"></i> EDIT</span></a>
                        </div>
                        </div>
                        ...
                        <div class="row">
                        <div class="col-md-6 col-sm-6 col-6 b-right">
                        <span class="cart-primary float-left removecartsession cartpageremove"  data-id="'.$i.'"> REMOVE</span>
                        </div>
                        <div class="col-md-6 col-sm-6 col-6">
                        '.$w.'
                        </div>
...
// public function wishlistdata(){
  ...
  function get_pricemobile(){
	...
	function minicart_data() {
        $cart_detailsviewdata = '';
        $cart_detailsviewdata = $cart_detailsviewdata.'<h3 style="color: #111;">My Cart ('.cart_count().') <span style="float:right">                                <button type="button" class="close" data-dismiss="modal" onclick=close_sidebar() aria-label="Close" style="z-index:99999999999999999!important;"><span aria-hidden="true">&times;</span></button></span></h3>
        <div class="product-cart-content">';

        $mini_cart_data = minicart_data();
        if(isset($_SESSION['user_id'])) {
            foreach ($mini_cart_data  as $cart_items):
                ...
                $slug = $product->slug;
                $t_id = json_decode($cart->template_id);
                $size= ($cart_items->size!="" && $cart->id==2)?'Size:&emsp;'.$cart_items->size:'';
                $img ='';
                if($t_id[0]!="" && $t_id[0]!=null) {
                    $key = related_product($t_id[0])->template_ids;
                    $data = json_decode($key);
                ...
                $cart_detailsviewdata  = $cart_detailsviewdata.'<div class="product-cart">
                        <div class="product-image">
                                '.$img.'
                        </div>
                        <div class="product-content">
                        <h3 class="p-name-text">
                        
                        '.ucwords(strtolower($cart->product_name)).'
                        
                        </h3>
                        
                         <span>'.$size.'</span>
                        <span>Qty:&emsp;&nbsp;&nbsp;'.$cart_items->qty.'</span>
                        ...
                        <div class="product-price">
                            <span style="display: block" class="price prodct-price-title"><b>'.currency().' '.$p->total.'/- </b></span>
<br/>
                            <div style="display: inline-flex">
                                <a href="'.base_url().'customizer?product='.$cart->id.'&cart_id='.$cart_items->id.'" style="color:green;display:inline;">
                                    <span class="cart-control" style="color:green"><i class="fas fa-edit"></i>EDIT</span>
                                </a>
                                    &emsp;&emsp;
                                    <a href="javascript: void(0)"><span class="cart-primary  removecart" data-cart_id ="'.$cart_items->id.'" data-rowid="'. $t_id[0].'" style="color:red;cursor:pointer;"><i class="fas fa-trash-alt"></i> REMOVE</span></a>
                                </div>
                            </div>
                        </div>
                        </div>';
            ...
            // $i = 0;
            foreach ($mini_cart_data as $i => $cart_items):
                $price = $cart_items["price"];
                $p = json_decode($price);
                $cart_data = $cart_items["cart_data"];
                $cart = json_decode($cart_data);
                ...
                //     </div>
                //     <div class="product-content">
                //     <h3 class="p-name-text">'.ucwords(strtolower($cart->product_name)).'</h3>
                //     <span>'.$size.'</span> 
                //     <span>Qty: &emsp; '.$cart_items["qty"].'</span>
                //     <div class="product-price">
                    
                    
                //     <span class="price prodct-price-title"><b> '.currency().' '.$p->total.' </b> </span>
                //     <div>
                    
                    
                //     <a href="'.base_url().'customizer?product='.$cart->id.'&cart_id='.$i.'" style="color:green;display:inline">
                //     <span class="cart-control" style="color:green"><i class="fas fa-edit"></i> EDIT</span>
                //     </a>
                    
                //     &emsp;&emsp;
                //     <span class="cart-primary  removecartsession withoutlogin"  data-id="'.$i.'" style="float:right; color:red;pointer:cursor;"><i class="fas fa-trash-alt"></i> REMOVE</span>
                ...
                $cart_detailsviewdata = $cart_detailsviewdata.'<div class="product-cart">
                        ...
                        <div class="product-price">
                            ...
                            <div style="display: inline-flex">
                                ...
                                &emsp;&emsp;
                                <a href="javascript: void(0)"><span class="cart-primary removecartsession withoutlogin" data-id="'.$i.'" style="color:red;pointer:cursor;"><i class="fas fa-trash-alt"></i> REMOVE</span></a>
                            </div>
                        </div>
                    </div>
                </div>';
        ...
        $cart_detailsviewdata = $cart_detailsviewdata.'</div>
            <div class="product-cart-subtotal">
            <span>Subtotal</span>
            <span class="subtotal">'.currency().' '.cart_total().'</span>
            </div>
            <div class="product-cart-btn">
            <a href="'.base_url().'cart" class="btn btn-dark btn-darkest">View  Cart</a>';
            //<a href="'.base_url().'cart" class="btn btn-light">View  Cart</a>';
            if(isset($_SESSION['user_id'])){
                  $cart_detailsviewdata = $cart_detailsviewdata.'<a data-toggle="modal" data-target="#condition_model" class="btn btn-dark btn-darkest" style="margin-top: 10px;">Checkout </a>';
                  }else{
                  $cart_detailsviewdata = $cart_detailsviewdata.'<a  onclick="guest_model()" class="btn btn-dark btn-darkest" style="margin-top: 10px;">Checkout</a>';
                  }
            $cart_detailsviewdata = $cart_detailsviewdata.'</div>';
        echo json_encode($cart_detailsviewdata);
	}
}