...
class lumise_connector {
    ...
    public function __construct() {
		global $lumise;
		$ci_databse_path = ROOT.'application/config/database.php';
		$ci_path = ROOT.'application/third_party/editor/ci_connector.php';
		include_once $ci_databse_path;
// 		$lumise->CI = require_once($ci_path);

        $this->platform = 'php';
        $url = SITE_URL;//$this->url(1);
        $core_uri = THEME_PATH;
        $core_url = SITE_URL.'/theme/';
        ...
        $this->config = array(
			"url" => $url,
			"tool_url" => $url.'admin_customizer',
			"logo" => $url. 'assets/images/logo1.jpg',
			"ajax_url" => $url.'customizer/ajax?',
			"admin_ajax_url" => $url.'admin_customizer/ajax?',
            "checkout_url" => $url.'admin_customizer/cart',
			"assets_url" => $core_url.'core/',
			"load_jquery" => true,
			"root_path" => dirname(__FILE__).DS.'core'.DS,

            "upload_path" => ROOT.'/uploads/',
			"upload_url" => $url.'uploads/',
            "admin_assets_url" => $core_url.'core/admin/assets/',
            "admin_url" => $url.'admin?',
	...
	public function admin_verify() {
		...
		if (!$this->is_admin()) {
			include ROOT.'theme'.DS.'login.php';
			exit;
	...
	public function process_reset() {
		...
		if (!lumise_secure::check_nonce('RESET-SECURITY', $nonce)) {
			...
			if ($limit[2] >= 5 && time()-$limit[3] < 60*60) {
				...
				$subject = 'AnshuWap - Reset control panel password';
	...
	public function add_to_cart($data){
		...
		global $lumise;
    	return $lumise->cfg->editor_url.'cart';
