...
class lumise_lib{
    ...
    public function get_products ($args) {
		...
		$query = array(
			...
			(!empty($args['category']) ?
				...
				"WHERE p.active = 1 AND cref.category_id = ".$args['category'].
	...
	public function get_product($id = null) {
		...
		global $lumise;
// 		$id = $lumise->esc_get('product', null);
// 		if ($id === null)
// 	    	return null;
// 		if ($id !== null)
// 			$id = (int)$id;
	...
	*/
	...
	public function product_cfg() {
		...
		if ($lumise->esc_get('share', false)) { //$_GET['share']
			
			$share = $lumise->lib->get_share($lumise->esc_get('share', false));
			
			if ($share === null || $share['product'] != $lumise->esc_get('product', false)) {
				echo ',';
				echo "\n";
				...
				if (!in_array($lumise->esc_get('share', false), $history)) {
					array_push($history, $lumise->esc_get('share', false));
