 ...
 *
 */
class my_share extends lumise_lib
{
    ...
    public function process() {
        ...
        foreach( $items_cart as $key => $item ) {
            ...
            //  $filename   = $this->save_cart_item_file( $item_data );
             $filename   = $this->save_item_file( $item_data );
...
// 			exit;
        ...
        foreach( $items_cart as $key => $item ) {
            ...
            // $filename   = $this->save_cart_item_file( $item_data );
             $filename   = $this->save_item_file( $item_data );
            $insert = $this->main->db->insert('shares_designs', array(
					'user_id' => (isset($_SESSION['user_id']))?$_SESSION['user_id']:'0',
					'file_url' => $filename,
					'share_url' =>'https://arteno3.anshuwap.com/customizer?product='.$item['id'].'&shares='.$filename,
				));
            if( $filename === false ){
        ...
        $_POST = array();
        $result=array(
            'share_url'=>$lumise->cfg->tool_url.'?product='.$pid.'&shares='.$filename,
            'screenshot'=>$screenshorts
            );
