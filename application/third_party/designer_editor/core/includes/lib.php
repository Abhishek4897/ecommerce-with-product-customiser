...
class lumise_lib{
	...
	public function get_xitems($category = '', $q = '', $index = 0, $type = 'cliparts', $limit = 48){
		...
		if($type == 'templates')
		    $item_sel = "item.id, item.product_base, item.name, item.description, item.author, item.screenshot, item.image, item.upload, item.featured, item.active, item.tags, item.order, item.created, item.updated, item.designed_by, item.favourite, item.color, item.slug, designer_charge.price";
		else
		    $item_sel = "item.*";
