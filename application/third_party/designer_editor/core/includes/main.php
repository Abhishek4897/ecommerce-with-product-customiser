...
define('LUMISE', '1.7.3.'.rand(000, 999));
...
class lumise {
	...
	public function init(){
		...
		if (
			...
			$this->db->prefix = isset($this->cfg->database['prefix']) ? $this->cfg->database['prefix'] : 'tbl_';
		...
		$this->router = $this->esc('my_customizer');//$this->esc('lumise-router');
	...
	public function esc($name = '', $default = '') {
	   $direct_allow = array("share1", "product");
	   if(in_array($name, $direct_allow))
	       return $this->esc_get($name, $default);
	   // $uris = explode('&', (explode('?', $_SERVER['REQUEST_URI'])[1]));
	   // $gets = array();
	   // foreach($uris as $url):
	   //     $url_exp = explode('=', $url);
	   //     $gets[$url_exp[0]] = $url_exp[1];
	   // endforeach;
	    $get = array_keys($_GET);
	    ...
	    if(!isset($get[0]))
	        return $default;
	    $uri = explode('/', $get[0]);
		return htmlspecialchars((isset($uri[1]) && $uri[1]==$name && isset($uri[2])) ? $uri[2] : $default);
	}


	public function esc_get($name = '', $default = '') {
	    $uri1 = explode('?', $_SERVER['REQUEST_URI']);
	    if(!isset($uri1[1]) || $uri1[1]==null)
	        return $default;
	    $uri = array();
	    if(isset($uri1[1]))
	        $uri = explode('&', ($uri1[1]));
	    ...
	    foreach($uri as $url):
	        $url_exp = explode('=', $url);
	        $get[$url_exp[0]] = $url_exp[1];
	    endforeach;
		return htmlspecialchars(isset($get[$name]) ? $get[$name] : $default);
	}
	...
	public function check_upload ($time = '') {
		...
		$args = $this->apply_filters('upload_folders', array('settings', 'cliparts', 'templates', 'products', 'thumbnails', 'user_uploads', 'fonts', 'orders', 'designs', 'shares', 'printings', 'logs', 'user_data', 'addons','share_data','wishlist_data','cart_data'));
		...
		if ($time !== '') {
			...
			$arym = $this->apply_filters('upload_folders', array('user_uploads', 'cliparts', 'templates', 'shares', 'designs', 'user_data', 'orders', 'share_data','wishlist_data','cart_data'));
