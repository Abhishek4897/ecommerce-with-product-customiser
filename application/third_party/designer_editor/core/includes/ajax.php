...
// 		}
	...
	public function publisher_cart() {
		...
		$move_file = file_put_contents($path.$file, $data);
        ...
        $lumise->connector->set_session( 'publish_cart', $cart_data );
        return true;
	}
	...
	public function upload_share_design() {
		...
		if ($this->main->connector->is_admin() || $this->main->cfg->settings['share'] == '1') {
			...
			$id = (isset($_POST['share_id']))? $_POST['share_id'] : $this->main->generate_id();
			...
			if (
				...
				file_put_contents($path.$id.$this->mytpl_ext, urldecode(base64_decode($_POST['data'])))
			) {
	...
	public function delete_link_share() {
		...
		if ($aid != $post_aid) {
			...
			if ($data == null || $data['aid'] != $aid) {
				...
				@unlink($path.$data['share_id'].$this->mytpl_ext);
	...
	public function wishlist() {
		...
		require_once(dirname(__FILE__).DS.'..'.DS.'wishlist.php');
	...
	public function share() {
		...
		require_once(dirname(__FILE__).DS.'..'.DS.'share.php');
	...
	?><!DOCTYPE html><html><head><title>Arteno Creation Private Limited.</title><script type="text/javascript">var fonts = {<?php
