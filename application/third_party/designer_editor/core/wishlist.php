 ...
 *
 */
class my_wishlist extends lumise_lib
{
    ...
    public function process_checkout() {
        ...
        $item = json_decode($item);
			...
			$tmp_charge=$template_charge['0'];
			$designer_charge=$tmp_charge['price'];
            if (isset($item->template)) {
                foreach ( $item->template->stages as $stage => $temp_ids ) {
                    if(
                        count($temp_ids) > 0
        ...
        foreach( $items_cart as $key => $item ) {
            ...
            // $filename   = $this->save_cart_item_file( $item_data );
             $filename   = $this->save_item_file( $item_data,"wishlist_data");
             $user_wishlist = array(
					'user_id' => $_SESSION['user_id'],
					'color' =>($item['options']->quantity)?$item['options']->quantity:null,
					'file' =>$filename,
					'temp_id' =>0,
					'base_id' =>$item['id'],
					'type' => 'custom',
					...
					'qty' =>($item['options']->quantity)?$item['options']->quantity:null,
					'wishlist_url' =>$lumise->cfg->tool_url.'?product='.$item['id'].'&wishlist='.$filename,
				);
				// print_r($user_wishlist);
            $insert = $this->main->db->insert('wishlist', $user_wishlist);
            if( $filename === false ){
        ...
        $_POST = array();
        $share_url=$lumise->cfg->url.'wishlist';
        echo $share_url;
    }
