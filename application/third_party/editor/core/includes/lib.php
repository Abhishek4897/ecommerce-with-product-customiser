...
class lumise_lib{
    ...
    public function x_items($type = 'cliparts') {
		...
		$category = htmlspecialchars(isset($_POST['category']) ? $_POST['category'] : 23);
		$index = (int)htmlspecialchars(isset($_POST['index']) ? $_POST['index'] : 0);
		$q = htmlspecialchars(isset($_POST['q']) ? $_POST['q'] : '');
		$limit = (int)htmlspecialchars(isset($_POST['limit']) ? $_POST['limit'] : 15);
		$cate_name = '';
...
// 		}
		...
		$cate_name = ($get_cate)?$get_cate['name']:"";
	...
	public function get_xitems($category = '', $q = '', $index = 0, $type = 'cliparts', $limit = 48){
        $base_id = isset($_POST['base_id']) ? htmlspecialchars($_POST['base_id']) : 2;
		...
		if($type == 'templates')
		    $item_sel = "item.id, item.product_base, item.name, item.description, item.author, item.screenshot, item.upload, item.featured, item.active, item.tags, item.order, item.created, item.updated, item.designed_by, item.favourite, item.color, item.slug, designer_charge.price";//, item.image
		else
		    $item_sel = "item.*";
		$select = $item_sel.", '$type' as resource ";
		$group_by = "";//"GROUP BY item.id"; //Manually comment for group_by issue resolve.
		$order_by = "ORDER BY `item`.`order` ASC"; //, `item`.`id` ASC
		$temp_where = "";
		if($type == 'templates')
            $temp_where = " and item.no_of_stages='1'";
...
// 			);
			...
			$query = array(
				...
				implode(' ', $from),
				"LEFT JOIN `{$this->main->db->prefix}designer_charge` designer_charge ON ".$base_id." = designer_charge.`p_id`",
				"WHERE item.product_base=".$base_id." ".$temp_where." and ".implode(' AND ', $where),
				(count($extra)>0? ' AND '.implode(' AND ', $extra):''),
				$group_by,
				$order_by
			);
			...
			if (count($cat_deactives)>0) {
				...
				$query = array(
					...
					"FROM {$this->main->db->prefix}{$type} item",
					"LEFT JOIN `{$this->main->db->prefix}designer_charge` designer_charge ON ".$base_id." = designer_charge.`p_id`",
					"WHERE item.active = 1 ".$temp_where." and item.product_base=".$base_id,
					(count($extra)>0? ' AND '.implode(' AND ', $extra):''),
					$group_by,
					$order_by
				);
    ...
    public function get_products ($args) {
		...
		$query = array(
			...
			(!empty($args['category']) ?
				...
				"WHERE cref.type='products' AND p.active = 1 AND cref.category_id = ".$args['category'].
				(!empty($args['s']) ? " AND p.name LIKE '%{$args['s']}%'" : '')
				:
				"WHERE p.active = 1".(!empty($args['s']) ? " AND p.name LIKE '%{$args['s']}%'" : '')
			),
			"ORDER BY `p`.`order`, `p`.`id` ASC",
			"LIMIT {$args['index']}, {$args['limit']}"
		);
		
		if ($args['no_cms_filter'] === true) {
	...
	public function prepare_product($product = array()) {
		...
		foreach ($product['attributes'] as $id => $attr) {
			...
			if (isset($attr->values) && !empty($attr->values)) {
				...
				foreach ($attr->values as $i => $val) {
					if (strpos($val, '|') !== false) {
						$val = explode('|', $val);
						$val[1] = $lumise->lang($val[1]);
						$val = implode('|', $val);
					}	
				}
				
				if ($attr->type == 'product_color') {
					$product['color'] = explode('|', $attr->values[0]);
		...
		$product['stages'] = $this->enjson($product['stages']);
		...
		if (!empty($prt) && $prt != '%7B%7D') {
		   ...
		   $cfg = !is_array($prt ) ? json_decode(str_replace(array('\\\''), array("'"), rawurldecode($prt)), true) : $prt;
		    ...
		    if (is_array($cfg)) {
				...
				$query = "SELECT * FROM `{$lumise->db->prefix}printings` WHERE id IN ($prints) ORDER BY field(id, $prints)";
	...
	*/
		...
		foreach ($cart_data['items'] as $key => $item){
			...
			if(
				...
				foreach ($extra_data['screenshots'] as $stage => $screenshot) {
					...
					$scr_file_name = date('Y'.DS.'m'.DS, time()).$lumise->generate_id().'.png';
			...
			if (false === $item['template']){
				...
				$design_file = date('Y'.DS.'m'.DS, time()).$lumise->generate_id().'.lumi';
	...
	public function product_cfg() {
		...
		if ($product !== null) {
			...
			echo ',';
			echo "\n";
			echo '				has_template :'.($has_template);
			echo ',';
			echo "\n";
	...
	public function save_cart_item_file( $data ) {
		...
		$filename = date('Y', time()).DS.date('m', time()).DS.$data[ 'cart_id' ] . '_' . $this->gen_str();
	...
	public function save_item_file( $data, $folder="share_data" ) {
		...
		$filename = date('Y', time()).DS.date('m', time()).DS.$this->gen_str();
		
		$path = $lumise->cfg->upload_path . $folder. DS . $filename . '.mytpl';
		$data[ 'screenshots' ] = base64_encode( json_encode( $data[ 'screenshots' ] ) );
		$content = json_encode( $data['design'] );
		$res = file_put_contents( $path, $content );
    ...
    public function get_product_colors() {
	    ...
	    $product = $this->main->db->rawQuery("SELECT `color` FROM `".$this->main->db->prefix."products` WHERE `id`=".(int)$_GET['id']);
    ...
    public function get_color($attrs = '') {
	    
	    $color = '#f0f0f0';
	    
	    if (isset($attrs) && !empty($attrs)) {
		    $attrs = $this->dejson($attrs);
		    if (is_object($attrs)) {
			    foreach ($attrs as $id => $attr) {
				    if ($attr->type == 'product_color' && !empty($attr->values)) {
					    $color = explode("\n", $attr->values);
