    ...
    public function process() {
        ...
        foreach ($this->data as $cart_id => $item) {
                ...
                foreach ( $item->template->stages as $stage => $temp_ids ) {
                    if(
		                ...
		                foreach ($temp_ids as $tem_id) {
		                    $imp_data=array_push($newarray,$tem_id);
			...
			$items_cart[ $cart_id ] = array(
                ...
                'product_name'  => $this->actual_product_name($item->product_name, $item->is_temp),
                'price' => array(
                    'total'     => 0,
                    'resource'  => 0,
        ...
        foreach( $items_cart as $key => $item ) {
            ...
            //  $filename   = $this->save_cart_item_file( $item_data );
             $filename   = $this->save_item_file( $item_data,"cart_data");
...
// 		$share_item = json_decode($_POST['data']);
        //loop through all cart items
...
// 			exit;
			...
			//BackSide Charge - only for product_id = 2(T-shirts).
			$backSideCharge = 0;
			$stage_data_count = 0;
		    $is_not_temp = 0;
            $is_temp = 0;
			...
			foreach ( $stages as $s => $stage ) {
                ...
                foreach ( $objects as $obj ){
                    if( 
                        ...
                        if(!isset($obj->template) && !isset($obj->template[0]))
                            $is_not_temp = 1;
                    }
                }
                $stage_data_count += $obj_data_count;
                if($stage_data_count>1 && $item->product_id==2)
                    $backSideCharge = 150; //Manually Added.
			}
            ...
            //template price
            $db = $lumise->get_db();
            $query = "SELECT * FROM `designer_charge` WHERE p_id = {$item->product_id}";
			$template_charge =  $db->rawQuery($query);
			$designer_charge = $template_charge['0']['price'];
            if (isset($item->template)) {
                foreach ( $item->template->stages as $stage => $temp_ids ) {
                    if(
                        count($temp_ids) > 0
		                ...
		                foreach ($temp_ids as $tem_id) {
	                    	$template = $this->get_template( $tem_id );
							$template_price += ( $template['price'] > 0 ) ? $template['price'] : $designer_charge;
	                        if($main_temp_id==0 || $item->product_id==3) //temporarly added.
	                            $main_temp_id = $tem_id;
	                        else
	                            $main_temp_id = 0;
		                }
			...
			//"options":{"DJ9d":"#ffffff","quantity":"1"},"printing":null,"product":3,"product_cms":3,"template":false
			$item->design->options = $item->options;
			$item->design->printing = $item->printing;
			$item->design->product = $item->product_id;
			$item->design->product_cms = $item->product_id;
			$item->design->template = $item->template;
			$item_is_temp = 0;
			$temp_data = "";
			if($is_temp == 1 && $is_not_temp==0) {
			    $item_is_temp = 1;
			    if($main_temp_id!=0)
			        $temp_data = $this->get_template($main_temp_id);
			 //   if($temp_data['color']!=$item->design->options->color)
			 //       $temp_data = "";
			}
			...
			$items_cart[ $cart_id ] = array(
                ...
                'product_id'    => $item->product_id,
                'product_cms'   => ( $lumise->connector->platform == 'php' )? $item->product_id : $item->cms_id,
                'product_name'  => $this->actual_product_name($item->product_name, $item_is_temp, $temp_data),
                'price' => array(
                    'total'     => 0,
                    'resource'  => 0,
                    'template'  => $template_price,
            ...
            foreach($item->template->stages as $front){
                if(is_array($front))
           ...
           $cart_details=array(
                ...
                'cart_id'       => $cart_id,
                'product_name'  => $this->actual_product_name($item->product_name, $item_is_temp, $temp_data),
                'template_id'      => (!empty($designBySingle))?json_encode($designBySingle):null,
                'screenshots'   => $screenshorts
                );
            unset($item);
        ...
        foreach( $items_cart as $key => $item ) {
            ...
            $qty        = $item['qty'];
            $sub_total  = 0;
            $sum        = $item[ 'price' ][ 'resource' ] + $item[ 'price' ][ 'base' ] + $item[ 'price' ][ 'template' ] + $item[ 'price' ][ 'side_charge' ];
            ...
            $item_data  =  $item;//$items_cart[ $key ];
            // $filename   = $this->save_cart_item_file( $item_data );
             $filename   = $this->save_item_file($item_data,"cart_data");
            
             if(isset($_POST['cart_id']) && $_POST['cart_id']!=""){
                 $arteno_cart_id=$_POST['cart_id'];
                $user_cart = array(
					'user_id' => (isset($_SESSION['user_id']))?$_SESSION['user_id']:'0',
					'color' => $item['options']->color,
					'file' =>$filename,
					'type' => 'custom',
					'cart_data' => json_encode(isset($cart_details)?$cart_details:""),
					...
					'size' =>($item['id']==2)?$item['options']->size:null,
					'qty' =>$item['options']->quantity
				);
				
                if(isset($_SESSION['user_id'])) {
                    $this->main->db->where ('id',$arteno_cart_id);
                    $this->main->db->update ('user_cart', $user_cart);
                } else {
                    $_SESSION['user_cart'][$arteno_cart_id]= $user_cart;
                }
             } else {
                $user_cart = array(
					'user_id' => (isset($_SESSION['user_id']))?$_SESSION['user_id']:'0',
         ...
         if(isset($_SESSION['user_id'])){
            $insert = $this->main->db->insert('user_cart', $user_cart);
         } else {
             if(isset($_SESSION['user_cart']) && is_array($_SESSION['user_cart'])){
                 $old_data=$_SESSION['user_cart'];
                 $updated_data=array_merge($old_data,array($user_cart));
                 $lumise->connector->set_session('user_cart', $updated_data );
             } else {
                 $updated_data[]=$user_cart;
                 $lumise->connector->set_session('user_cart', $updated_data );
             }
         }  
             }
		...
		echo $lumise->cfg->editor_url.'cart';
    }
    ...
    public function actual_product_name($prod_name, $item_is_temp, $temp_data="") {
        if($temp_data!="" && $item_is_temp==1)
            $name = $temp_data['name'];
        else
            $name = substr($prod_name, 0, -1);
        return $name;
    }
